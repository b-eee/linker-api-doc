...
```text
include_date_at=true とすると結果にデータ作成日(created_at)、データ更新日(updated_at)を含める
```
...
```text
POST https://api.xxx.com/api/v0/applications/APP_ID/reports/REPORT_ID/filter
...
```javascript
{
    ...
    "include_date_at": true,  // 結果にデータ作成日(created_at)、データ更新日(updated_at)を含める
    "format": "csv"　 // 結果をCSV形式で取得する
}
```
created_at, updated_at について、日本語の場合は「データ作成日」、「データ更新日」　英語の場合は "created_at", "updated_at"

**Response Sample**
※（JSON形式は、省略） 形式は、データレポート取得サンプルと同様
※以下は、CSV形式の場合のサンプル

```text
Number,カテゴリ,ステータス,データ作成日時,データ更新日時
45,A,受付,2020/08/02 17:53:42,2020/08/13 00:09:52
11,B,新規,2020/08/02 17:53:42,2020/08/16 04:07:16
